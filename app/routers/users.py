...
async def create_admin_user_route(data: UserCreate,db: Session = Depends(get_db), admin = Depends(get_current_admin_user)):
    """
...
async def change_password_route(new_password:UserUpdate, user=Depends(get_current_user),db: Session = Depends(get_db)):
    """
